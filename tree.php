<?php
// ini_set('open_basedir', dirname('/Users/vicent/Sites/git/lab/') . DIRECTORY_SEPARATOR);
...
if(isset($_GET['operation'])) {
	$fs = new fs('/Users/vicent/Sites/git/lab' . DIRECTORY_SEPARATOR);
	try {
		$rslt = null;
...
<html>
	<head>
		...
		#tree .folder { background:url('./jstree/file_sprite.png') right bottom no-repeat; }
		#tree .file { background:url('./jstree/file_sprite.png') 0 0 no-repeat; }
		...
		.border:hover { border:1px solid red;box-shadow: 0 0 5px #bb0000;}
		#biao1 {height:30px; text-align: center;}
		</style>
	</head>
	<body>
		<div id="container" role="main">
			...
			<div id="data">
				...
				<span id='size'></span>
					<textarea cols="10" rows="10" style="" id="biao1"></textarea>
				</div>
							...
							if(d && typeof d.type !== 'undefined') {
								...
								switch(d.type) {
									...
									case 'html':
										$('#data .code').show();
										$('#code').val(d.content);
										console.log('tree.php>>>426', data.selected);
										break;
									case 'png':
									case 'jpg':
									case 'jpeg':
									...
									case 'gif':
										var _width;
										var _height;
										var image = $('#data .image img');

										$('#data .image img').one('load', function () {
											$('#data .image .hover').remove();

											_width = $(this).height()/2;
											_height = $(this).width()/2;

											var scale = image[0].clientWidth / image[0].naturalWidth;

											$(this).css({'marginTop':'-' + _width + 'px','marginLeft':'-' + _height + 'px'});


											// console.log('tree.php>>>438', image);


											let size = 'width:' + image[0].clientWidth + 'px' + ';height:' + image[0].clientHeight + 'px;';
											let margin = 'marginTop:' + _width + 'px;' + 'marginLeft:' + _height + 'px;'

											$('#data .image').append("<div class='hover' style='" + size + margin + ' ' + image[0].style.cssText + "'></div>");

											// $('#data .image .hover').append("<div class='border' style='"   + ' '  + "'>Hello</div>");

											if(d.type === 'png') {
												$.get('?operation=get_content&id=' + data.selected.join(':').replace(/.png/, '.json'), function(_d) {
													if(_d.content) {
														var frames = JSON.parse(_d.content).frames;
														for (var frame in frames) {
															var borderSize = 'width:' + frames[frame].frame.w * scale  + 'px' + ';height:' + frames[frame].frame.h * scale + 'px;';

															let borderMargin = 'margin-top:' + frames[frame].frame.y * scale + 'px;' + 'margin-left:' + frames[frame].frame.x * scale+ 'px;'
															$('#data .image .hover').append("<a " + "data-size=" + frames[frame].frame.w + 'x' + frames[frame].frame.h + ' href="javascript: void(0);" title=' + frame + " class='border' style='" + borderMargin  + borderSize  + ' ' + "'></a>");


															console.log('tree.php>>>460', frames[frame].frame.w);
														}
														$('#data .image .hover a').on('click', function(e) {
															var Url2 = document.getElementById("biao1");
															Url2.value = e.currentTarget.title;
															Url2.select(); // 选择对象
															document.getElementById("size").innerHTML = e.currentTarget.dataset.size
															document.execCommand("Copy"); // 执行浏览器复制命令
															console.log('tree.php>>>486', e.currentTarget);
														})
													}
												})
											}
										...
										// $('#data .image .hover')[0].css({'marginTop':'-' + _width + 'px','marginLeft':'-' + _height + 'px'});

										// $('#data .image .hover')[0].style.marginLeft = _width;
										// $('#data .image .hover')[0].style.marginTop = _height;


										console.log('tree.php>>>443', $('#data .image .hover'));
